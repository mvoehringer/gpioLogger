...
# Default-Stop:         0 1 6
# Short-Description:    gpioLogger for volkszaehler.org
...
NODE_PATH=
USER=gpiologger 			# USER who OWNS the daemon process (no matter whoever runs the init script)
APPLICATION_DIRECTORY="/opt/gpioLogger"
APPLICATION_START=gpioLogger.js
PIDFILE=/home/gpiologger/gpioLogger.pid
LOGFILE=/var/log/gpiologger/gpioLogger.log
...
start() {
    ...
    su - $USER -c "NODE_ENV=production forever --pidFile $PIDFILE --sourceDir $APPLICATION_DIRECTORY \
        -a -e $LOGFILE --minUptime 5000 --spinSleepTime 2000 \
...
stop() {
    if [ -f $PIDFILE ]; then
        ...
        su - $USER -c "NODE_ENV=production forever stop $APPLICATION_START"
